...
const UserLevelInfo = ({ targetUserId }: Props) => {
    ...
    const progress = userLevel ? (userLevel.currentLevelExp / userLevel.levelUpExpCost) * 100 : 0;
    return (
        <Group w={"100%"}>
            <Stack w={"100%"} gap={2}>
                <Group justify={"space-between"} w={"100%"}>
                    ...
                    <Text>
                        {userLevel?.currentLevelExp ?? 0} / {userLevel?.levelUpExpCost ?? 0} XP
                    </Text>
                </Group>
                <Progress className={"w-full border-2 border-[#9A9A9A]"} size="lg" value={progress} />
            </Stack>
        </Group>
    );
};
